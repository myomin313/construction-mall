...
<div class="company_services">->
 <section class="suplier inner-heading" style="background-image: url('{{ asset('storage/company_coverphoto/'.$projectsetting->service_list_image )}}')">
    <div class="container">
    <h1>{{ $category_name }}</h1>
      <ul class="xs-breadcumb">
        <li><a href="{{ url('/')}}" style="color: {{ $projectsetting->service_nav_first_background_and_icon }}">Home /</a>  {{ $category_name }}</li>
      </ul>
    </div>
  </section>
<!--search form start-->
           ...
           <div class="advanced_search">
                        ...
                        <i class="fa fa-folder-open"></i>
                          <select class="form-control" name="category">
                              ...
                              @foreach($serviceCategories as $serviceCategory)
                               @php
                                      if($category == $serviceCategory->id || $id == $serviceCategory->id)
                                          $choose = "selected";
                                      else
                                          $choose = "";
                                  @endphp
                       <option value="{{ $serviceCategory->id }}" {{$choose}}>{{ $serviceCategory->name }}</option>
                      @endforeach
                            </select>
                      </div>
                     <div class="form-group col-md-3 inner-addon left-addon ">
              ...
              @endforeach
                      ...
                      <div class="form-group col-md-2 col-sm-12">
                        <input type="submit" name="" class="btn btn-sm advanced_search_btn" value="Search" style="background:linear-gradient(90deg, {{ $projectsetting->	service_nav_first_background_and_icon }} 50%, {{ $projectsetting->service_nav_second_background }}) !important;color:{{ $projectsetting->service_nav_font_color}} !important">
                      </div>
              </form>
              <!--Row End-->
          </div>
            ...
            <div class="blog-inter">
              <div class="row">
                ...
                <div class="col-md-8 col-sm-8">
                    ...
                    <ul class="blogDate">
                      <li> <i class="fa fa-user" aria-hidden="true" style="color:{{ $projectsetting->	service_nav_first_background_and_icon }} !important;"></i><span>
                    Service Company</span> </li>
                      <li> <i class="fa fa-envelope" aria-hidden="true" style="color:{{ $projectsetting->	service_nav_first_background_and_icon }} !important;"></i> <span><a href="mailto:{{ $data->email }}">{{ $data->email }}</a></span> </li>
                      <li> <i class="fa fa-phone" aria-hidden="true" style="color:{{ $projectsetting->	service_nav_first_background_and_icon }} !important;"></i> <span><a href="tel:{{ $data->phone }}">{{ $data->phone }}</a></span> </li>
                    </ul>
                    <ul class="blogDate">
                      <li> <i class="fa fa-home" aria-hidden="true" style="color:{{ $projectsetting->	service_nav_first_background_and_icon }} !important;"></i><span> <?php
        ...
        <li class="col-md-3 col-sm-12">
          <div class="side-bar">
            ...
            <div class="side-barBox">
              <h5 class="side-barTitle" style="background:linear-gradient(90deg, {{ $projectsetting->service_nav_first_background_and_icon }} 50%, {{ $projectsetting->service_nav_second_background }}) !important;color:{{ $projectsetting->service_nav_font_color}} !important">Categories</h5>
              <ul class="categories">
                 @foreach($serviceCategories as $servicecategory)
       <li class="{{ $servicecategory->name == $category_name ? 'active' : '' }}"><a  href="{{ url('companies/service/'.$servicecategory->category_url) }}">{{ $servicecategory->name }}</a></li>
    @endforeach
              </ul>
            </div>
          </div>
