              ...
              @else
                <div style="float: left;width:80px">
                  <form action="{{ route('logout') }}" method="POST" style="background:black">
                      @csrf
                   <button type="submit" style="background:black;color:white" data-dismiss="modal" aria-label="Close">Logout</button>
                 </form>
                 </div>
                  @if(auth()->user()->role == 1)
                     <div style="float: left;">
                   <a href="{{ url('user/dashboard') }}" type="button"  data-dismiss="modal" aria-label="Close"  style="background:black;color:white">Dashboard</a>
                  </div>
                  @elseif(auth()->user()->role == 2)
                  <div style="float: left;">
                   <a href="{{ url('company/dashboard') }}" type="button"  data-dismiss="modal" aria-label="Close"  style="background:black;color:white">Dashboard</a>
                  </div>
                   @elseif(auth()->user()->role == 3)
                   <div style="float: left;">
                   <a href="{{ url('freelancer/profile') }}" type="button"  data-dismiss="modal" aria-label="Close"  style="background:black;color:white">Dashboard</a>
                  </div>
                  @else
                  <div style="float: left;">
                   <a href="{{ url('admin/companies') }}" type="button"  data-dismiss="modal" aria-label="Close"  style="background:black;color:white">Dashboard</a>
                  </div> 
                  @endif
              @endif
